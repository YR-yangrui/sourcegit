...
namespace SourceGit.Models
{
    public class Watcher : IDisposable
    {
        ...
        public void MarkBranchUpdated()
        {
            ...
            Interlocked.Exchange(ref _dirtyWC, 0);
        }
        ...
        private void Tick(object sender)
        {
            ...
            if (oldUpdateBranch > 0)
            {
                if (now > oldUpdateBranch)
                {
                    ...
                    {
                        _repo.MarkBranchesDirty();
                        _repo.MarkWorktreesDirty();
                        _repo.MarkCommitsDirty();
                        _repo.MarkWorkingCopyDirty();

                        if (_repo.MayHaveSubmodules())
                            _repo.MarkSubmodulesDirty();
                    }
                }
                else
                {
            ...
            var oldDirtyWC = Interlocked.Exchange(ref _dirtyWC, -1);
            if (oldDirtyWC > 0)
            {
                if (now > oldDirtyWC)
                {
                    _repo.MarkWorkingCopyDirty();
                    if (_repo.IsActive && _repo.IsForeground)
                        _repo.RefreshWorkingCopyChangesIfDirty();
                }
                else
                    Interlocked.CompareExchange(ref _dirtyWC, oldDirtyWC, -1);
            }
        ...
        private void HandleWorkingCopyFileChanged(string name, string fullpath)
        {
            ...
            if (name.Equals(".gitmodules", StringComparison.Ordinal))
            {
                var desired = DateTime.Now.AddSeconds(1).ToFileTime();
                Interlocked.Exchange(ref _updateSubmodules, desired);
                Interlocked.Exchange(ref _dirtyWC, desired);
                return;
            }
            ...
            Interlocked.Exchange(ref _dirtyWC, DateTime.Now.AddSeconds(1).ToFileTime());
        }
