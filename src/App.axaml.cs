...
namespace SourceGit
{
    public partial class App : Application
    {
        ...
        public static void Main(string[] args)
        {
            ...
            Diagnostics.DiagnosticManager.Setup(Native.OS.DataDir);

            AppDomain.CurrentDomain.UnhandledException += (_, e) =>
            {
                var ex = e.ExceptionObject as Exception;
                Diagnostics.DiagnosticManager.Error("App", "app.unhandled_exception", "Unhandled domain exception", ex);
                Native.OS.LogException(ex);
            };

            TaskScheduler.UnobservedTaskException += (_, e) =>
            {
                Diagnostics.DiagnosticManager.Error("App", "app.unobserved_task_exception", "Unobserved task exception", e.Exception);
                e.SetObserved();
            };
            ...
            {
                Diagnostics.DiagnosticManager.Error("App", "app.startup_failed", "Top-level startup failure", ex);
                Native.OS.LogException(ex);
            }
            finally
            {
                Diagnostics.DiagnosticManager.Shutdown(ViewModels.Preferences.Instance.ExportPerfettoTraceOnExit);
            }
        }
        ...
        private void Check4Update(bool manually = false)
        {
            Task.Run(async () =>
            {
                ...
                {
                    var pref = ViewModels.Preferences.Instance;
                    var channel = Models.UpdateChannels.Normalize((Models.UpdateChannel)pref.UpdateChannel);
                    ...
                    if (!manually)
                    {
                        if (result is Models.UpdateAvailable update && pref.IsUpdateIgnored(update.Channel, update.TagName))
                            return;
                    }
