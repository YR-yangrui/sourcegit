...
namespace SourceGit.ViewModels
{
    ...
    public class Merge : Popup
    {
        ...
        public bool FindRenames
        {
            get => _findRenames;
            set
            {
                if (SetProperty(ref _findRenames, value))
                    Test();
            }
        }

        public double FindRenamesSimilarity
        {
            get => _findRenamesSimilarity;
            set
            {
                var normalized = System.Math.Clamp(value, 0, 100);
                if (SetProperty(ref _findRenamesSimilarity, normalized))
                    Test();
            }
        }
        ...
        public Merge(Repository repo, Models.Branch source, string into, bool forceFastForward)
        {
            ...
            _enableConflictTest = !forceFastForward;
            if (_enableConflictTest)
                Test();
        }
        ...
        public override async Task<bool> Sure()
        {
            ...
            var succ = await new Commands.Merge(
                _repo.FullPath,
                _sourceName,
                Mode.Arg,
                _canEditMessage && Edit,
                GetFindRenamesStrategyOption())
                .Use(log)
                .ExecAsync();
        ...
        private void Test()
        {
            if (!_enableConflictTest || Native.OS.GitVersion < Models.GitVersions.TESTING_MERGE)
                return;

            var testingId = ++_testingId;
            var strategyOption = GetFindRenamesStrategyOption();
            TestingState = MergeTestingState.Testing;

            Task.Run(async () =>
            {
                var exitCode = await new Commands.MergeTree(_repo.FullPath, _sourceName, Into, strategyOption)
                    .GetExitCodeAsync()
                    .ConfigureAwait(false);

                Dispatcher.UIThread.Post(() =>
                {
                    if (testingId != _testingId)
                        return;
        ...
        private string GetFindRenamesStrategyOption()
        {
            return FindRenames ? $"find-renames={(int)System.Math.Round(FindRenamesSimilarity)}%" : "no-renames";
        }
        ...
        private bool _findRenames = true;
        private bool _enableConflictTest = true;
        private MergeTestingState _testingState = MergeTestingState.Disabled;
        private double _findRenamesSimilarity = 50;
        private int _testingId = 0;
    }
}
