...
namespace SourceGit.ViewModels
{
    ...
    public class RepositoryNode : ObservableObject
    {
        ...
        public async Task UpdateStatusAsync(bool force, CancellationToken? token)
        {
            ...
            var repoPath = SourceGit.Diagnostics.DiagnosticManager.GetRepositoryPath(_id);
            using var span = SourceGit.Diagnostics.DiagnosticManager.StartSpan(
                "Repository.Status",
                "repository_node.update_status",
            ...
            if (!force)
            {
                ...
                if (passed.TotalSeconds < 10.0)
                {
                    ...
                    span.Set("skipReason", "throttled");
                    return;
                }
            }

            _lastUpdateStatus = DateTime.Now;
            var includeUntracked = LoadIncludeUntrackedInLocalChanges();
            span.Set("includeUntracked", includeUntracked);
            var status = await new Commands.QueryRepositoryStatus(_id, includeUntracked).GetResultAsync();
            Status = status;
            span.Set("success", status != null);
            if (status != null)
            {
        ...
        private bool LoadIncludeUntrackedInLocalChanges()
        {
            var gitDir = Commands.QueryGitDir.GetRepositoryGitDir(_id);
            if (string.IsNullOrEmpty(gitDir))
                return true;

            return Models.RepositoryUIStates.Load(gitDir).IncludeUntrackedInLocalChanges;
        }
