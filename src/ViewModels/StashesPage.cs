...
namespace SourceGit.ViewModels
{
    public class StashesPage : ObservableObject, IDisposable
    {
                ...
                if (SetProperty(ref _selectedStash, value))
                {
                    ...
                    else
                    {
                        Task.Run(async () =>
                        {
                            ...
                            {
                                if (!_disposed && value.SHA.Equals(_selectedStash?.SHA ?? string.Empty, StringComparison.Ordinal))
                                {
        ...
        public List<Models.Stash> SelectedStashes
        {
            get => _selectedStashes;
            set
            {
                if (SetProperty(ref _selectedStashes, value))
                    OnPropertyChanged(nameof(SelectedStashesCount));
            }
        }

        public int SelectedStashesCount
        {
            get => _selectedStashes.Count;
        }
            ...
            set
            {
                ...
                if (SetProperty(ref _selectedChanges, value))
                {
                    if (value is not { Count: 1 })
                        ...
                        DiffHost.ShowDiff(_repo.FullPath, Models.DiffOption.ForStash(_selectedStash.UntrackedParent, _selectedStash.Parents[2], value[0], true), "stash.select_untracked");
                    else
                        DiffHost.ShowDiff(_repo.FullPath, Models.DiffOption.ForStash(_selectedStash.Parents[0], _selectedStash.SHA, value[0], false), "stash.select");
                }
            }
        }
        ...
        public void Drop(List<Models.Stash> stashes)
        {
            if (stashes is { Count: > 0 } && _repo.CanCreatePopup())
                _repo.ShowPopup(new DropStash(_repo, stashes));
        }
        ...
        public async Task SaveStashAsPatchAsync(Models.Stash stash, string saveTo)
        {
            ...
            foreach (var c in changes)
                opts.Add(Models.DiffOption.ForStash(stash.Parents[0], stash.SHA, c, false));
            ...
            if (stash.Parents.Count == 3)
            {
                ...
                foreach (var c in untracked)
                    opts.Add(Models.DiffOption.ForStash(stash.UntrackedParent, stash.Parents[2], c, true));
        ...
        public void OpenChangeWithExternalDiffTool(Models.Change change)
        {
            ...
            if (_untracked.Contains(change))
                opt = Models.DiffOption.ForStash(_selectedStash.UntrackedParent, _selectedStash.Parents[2], change, true);
            else
                opt = Models.DiffOption.ForStash(_selectedStash.Parents[0], _selectedStash.SHA, change, false);
        ...
        public async Task ApplySelectedChanges(List<Models.Change> changes)
        {
            ...
            {
                if (_untracked.Contains(c) && _selectedStash.Parents.Count == 3)
                    opts.Add(Models.DiffOption.ForStash(_selectedStash.UntrackedParent, _selectedStash.Parents[2], c, true));
                else
                    opts.Add(Models.DiffOption.ForStash(_selectedStash.Parents[0], _selectedStash.SHA, c, false));
            }
        ...
        private List<Models.Stash> _selectedStashes = [];
