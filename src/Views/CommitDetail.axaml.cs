...
namespace SourceGit.Views
{
    public partial class CommitDetail : UserControl
    {
        ...
        public ContextMenu CreateMultipleChangesContextMenu(List<Models.Change> changes, List<string> selectedPaths = null)
        {
            if (DataContext is not ViewModels.CommitDetail { Repository: { } repo, Commit: { } commit } vm)
                return null;
            ...
            copyPath.Click += async (_, ev) =>
            {
                ...
                if (paths.Count == 1)
                {
                    await this.CopyTextAsync(paths[0]);
                }
                else
                {
            ...
            copyFullPath.Click += async (_, e) =>
            {
                ...
                if (paths.Count == 1)
                {
                    await this.CopyTextAsync(Native.OS.GetAbsPath(repo.FullPath, paths[0]));
                }
                else
                {
                    var builder = new StringBuilder();
                    foreach (var path in paths)
                        builder.AppendLine(Native.OS.GetAbsPath(repo.FullPath, path));
        ...
        public ContextMenu CreateChangeContextMenu(Models.Change change)
        {
            ...
            history.Click += (_, ev) =>
            {
                this.ShowWindow(new ViewModels.FileHistories(repo, change.Path, commit.SHA));
                ev.Handled = true;
            };
        ...
        private void OnDetailsTabStripPointerPressed(object sender, PointerPressedEventArgs e)
        {
            this.FindAncestorOfType<Histories>()?.OnDetailsTabStripPointerPressed(sender, e);
        }

        private void OnDetailsTabStripPointerMoved(object sender, PointerEventArgs e)
        {
            this.FindAncestorOfType<Histories>()?.OnDetailsTabStripPointerMoved(sender, e);
        }

        private void OnDetailsTabStripPointerReleased(object sender, PointerReleasedEventArgs e)
        {
            this.FindAncestorOfType<Histories>()?.OnDetailsTabStripPointerReleased(sender, e);
        }
