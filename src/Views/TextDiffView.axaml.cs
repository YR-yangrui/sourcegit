...
namespace SourceGit.Views
{
    public class ThemedTextDiffPresenter : TextEditor
    {
        ...
        private void OnTextViewContextRequested(object sender, ContextRequestedEventArgs e)
        {
            ...
            if (DataContext is ViewModels.TextDiffContext { SupportsChunkActions: true })
            {
        ...
        {
            if (DataContext is not ViewModels.TextDiffContext { SupportsChunkActions: true, Option: { IsLocalChange: true } })
                return;
        ...
        protected void LoadDocumentAsync(object expectedDataContext, List<Models.TextDiffLine> lines)
        {
            CancelDocumentBuild();

            var requestId = ++_documentBuildRequestId;
            var cancellation = new CancellationTokenSource();
            _documentBuildCancellation = cancellation;

            Document = new TextDocument();

            _ = Task.Run(() => BuildDocument(lines, cancellation.Token), cancellation.Token)
                .ContinueWith(task =>
                {
                    if (task.IsCanceled)
                    {
                    ...
                    if (task.IsFaulted)
                    {
                        Native.OS.LogException(task.Exception);
                        cancellation.Dispose();
                        return;
                    }
                    ...
                    Dispatcher.UIThread.Post(() =>
                    {
                        try
                        {
                            if (requestId != _documentBuildRequestId ||
                                !ReferenceEquals(_documentBuildCancellation, cancellation) ||
                                !ReferenceEquals(DataContext, expectedDataContext))
                                return;
                        ...
                        {
                            if (ReferenceEquals(_documentBuildCancellation, cancellation))
                                _documentBuildCancellation = null;
        ...
        private void CancelDocumentBuild()
        {
            _documentBuildRequestId++;
            _documentBuildCancellation?.Cancel();
            _documentBuildCancellation = null;
        }

        private static TextDocument BuildDocument(List<Models.TextDiffLine> lines, CancellationToken cancellationToken)
        {
            ...
            for (var i = 0; i < lines.Count; i++)
            {
                ...
                AppendDocumentLine(builder, lines[i]);
            }
        ...
        private static void AppendDocumentLine(StringBuilder builder, Models.TextDiffLine line)
        {
        ...
        {
            if (DataContext is not ViewModels.TextDiffContext { SupportsChunkActions: true, Data: { } diff, Option: { } option } ctx)
                return;
        ...
        private CancellationTokenSource _documentBuildCancellation;
        private int _documentBuildRequestId;
    }
    ...
    public class CombinedTextDiffPresenter : ThemedTextDiffPresenter
    {
        ...
        protected override void OnDataContextChanged(EventArgs e)
        {
            ...
            {
                LoadDocumentAsync(DataContext, diff.Lines);
            }
            else
            {
                ClearDocument();
    ...
    public class SingleSideTextDiffPresenter : ThemedTextDiffPresenter
    {
        ...
        protected override void OnDataContextChanged(EventArgs e)
        {
            ...
            if (DataContext is ViewModels.TwoSideTextDiff diff)
            {
                ...
                LoadDocumentAsync(DataContext, lines);
            }
            else
            {
                ClearDocument();
    ...
    public partial class TextDiffView : UserControl
    {
        ...
        {
            if (DataContext is not ViewModels.TextDiffContext { SupportsChunkActions: true, SelectedChunk: { } chunk, Data: { } diff, Option: { } option } vm)
                return;
