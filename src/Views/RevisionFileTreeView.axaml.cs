...
namespace SourceGit.Views
{
    ...
    public class RevisionFileRowsListBox : ListBoxEx
    {
        ...
        protected override async void OnKeyDown(KeyEventArgs e)
        {
            ...
            if (SelectedItem is ViewModels.RevisionFileTreeNode node)
            {
                ...
                else if (e.Key is Key.W or Key.S && e.KeyModifiers == KeyModifiers.None)
                {
                    var next = FindVisibleFile(node, e.Key == Key.W ? -1 : 1);
                    if (next != null)
                        Select(next);
        ...
        private ViewModels.RevisionFileTreeNode FindVisibleFile(ViewModels.RevisionFileTreeNode item, int step)
        {
            var idx = Items.IndexOf(item);
            if (idx == -1)
                return null;

            for (var i = idx + step; i >= 0 && i < Items.Count; i += step)
            {
                if (Items[i] is ViewModels.RevisionFileTreeNode { IsFolder: false } node)
                    return node;
            }
    ...
    public partial class RevisionFileTreeView : UserControl
    {
        ...
        public async Task SetSearchResultAsync(string file, bool preserveCurrentSearchTarget = false)
        {
            ...
            var searchResult = new List<ViewModels.RevisionFileTreeNode>();
            if (string.IsNullOrEmpty(file))
            {
                _isSearchResultMode = false;
                _searchResultPath = string.Empty;
            ...
            else
            {
                ...
                var target = preserveCurrentSearchTarget && !string.IsNullOrEmpty(_searchResultPath) ? _searchResultPath : file;
                var objects = await vm.GetRevisionObjectsAsync(target);
                if (!IsCurrentSearchRequest(revision, requestId))
                    return;

                if (!preserveCurrentSearchTarget && objects is not { Count: 1 } && vm.RevisionFileSearchSuggestion is { Count: > 0 })
                {
                    target = vm.RevisionFileSearchSuggestion[0];
                    objects = await vm.GetRevisionObjectsAsync(target);
                    if (!IsCurrentSearchRequest(revision, requestId))
                        return;
                }

                if (objects is not { Count: 1 } || objects[0].Type == Models.ObjectType.Tree)
                {
                    _isSearchResultMode = true;
                    _searchResultPath = target;
                    _selectedPath = string.Empty;
                }
                else
                {
                    _isSearchResultMode = true;
                    _selectedPath = objects[0].Path;
                    _searchResultPath = objects[0].Path;
                    BuildSearchResultTree(searchResult, objects[0]);
                    MakeRows(rows, searchResult, 0);
                }
            }
            ...
            {
                _searchResult.Clear();
                _searchResult.AddRange(searchResult);
                Rows.Clear();
                Rows.AddRange(rows);
            }
            finally
            ...
            await RestoreRowsStateAsync(DataContext as ViewModels.CommitDetail);
        }
        ...
        public async Task ToggleNodeIsExpandedAsync(ViewModels.RevisionFileTreeNode node)
        {
            ...
            try
            {
                var requestId = _reloadRequestId;
                var revision = GetCurrentRevision();
                ...
                {
                    var vm = DataContext as ViewModels.CommitDetail;
                    var subtree = await GetChildrenOfTreeNodeAsync(vm, node);
                    if (!IsCurrentReloadRequest(revision, requestId))
                        return;

                    if (!_isSearchResultMode && !string.IsNullOrEmpty(node.Backend?.Path))
                        _expandedPaths.Add(node.Backend.Path);

                    idx = Rows.IndexOf(node);
                    if (idx == -1)
                        return;
                ...
                else
                {
                    ...
                    if (!_isSearchResultMode && !string.IsNullOrEmpty(node.Backend?.Path))
                        _expandedPaths.Remove(node.Backend.Path);
                }
            }
            finally
            {
        ...
        private async void OnRowsSelectionChanged(object sender, SelectionChangedEventArgs _)
        {
            ...
            {
                _selectedPath = node.Backend?.Path ?? _selectedPath;
        ...
        public async Task ReloadAsync(bool restoreRowsState, bool updateRows)
        {
            if (DataContext is ViewModels.CommitDetail vm)
                await ReloadAsync(vm, restoreRowsState, updateRows);
        }

        private async Task<List<ViewModels.RevisionFileTreeNode>> GetChildrenOfTreeNodeAsync(ViewModels.CommitDetail vm, ViewModels.RevisionFileTreeNode node)
        {
            if (!node.IsFolder)
                return null;
            ...
            if (vm == null || string.IsNullOrEmpty(node.Backend?.SHA))
                return null;

            var objects = await vm.GetRevisionTreeChildrenAsync(node.Backend.SHA, node.Backend.Path);
            foreach (var obj in objects)
                node.Children.Add(new ViewModels.RevisionFileTreeNode { Backend = obj, Depth = node.Depth + 1 });
        ...
        private async Task ReloadAsync(ViewModels.CommitDetail vm, bool restoreRowsState = true, bool updateRows = true)
        {
            var requestId = ++_reloadRequestId;
            _searchRequestId++;
            var searchRequestId = _searchRequestId;
            var revision = vm?.Commit?.SHA ?? string.Empty;

            if (vm?.Commit == null || string.IsNullOrEmpty(revision))
            {
                ClearRows(false);
                return;
            }
            ...
            CaptureScrollOffset();
            var rootTreeSHA = await vm.GetRevisionRootTreeSHAAsync();
            if (!IsCurrentReloadRequest(revision, requestId))
                return;

            if (searchRequestId != _searchRequestId && _isSearchResultMode)
                return;

            if (string.IsNullOrEmpty(rootTreeSHA))
                return;
            ...
            _tree.Clear();
            _searchResult.Clear();
            var availableExpandedPaths = new HashSet<string>();
            var tree = await CreateNodesAsync(vm, rootTreeSHA, string.Empty, 0, availableExpandedPaths, revision, requestId);
            if (!IsCurrentReloadRequest(revision, requestId))
                return;

            _tree.AddRange(tree);
            _expandedPaths.RemoveWhere(path => !availableExpandedPaths.Contains(path));
            ...
            if (updateRows)
            {
                ...
                if (restoreRowsState)
                    await RestoreRowsStateAsync(vm);
            }
        }

        private void ClearRows(bool clearExpandedPaths)
        {
            _isSearchResultMode = false;
            _searchResultPath = string.Empty;
            _tree.Clear();
        ...
        private bool IsCurrentReloadRequest(string revision, long requestId)
        {
            return requestId == _reloadRequestId && string.Equals(GetCurrentRevision(), revision, StringComparison.Ordinal);
        }

        private bool IsCurrentSearchRequest(string revision, long requestId)
        {
            return requestId == _searchRequestId && string.Equals(GetCurrentRevision(), revision, StringComparison.Ordinal);
        }
        ...
        {
            if (RowsList?.FindDescendantOfType<ScrollViewer>() is { } scroller)
                _scrollOffset = scroller.Offset;
        }

        private async Task RestoreRowsStateAsync(ViewModels.CommitDetail vm)
        {
            var selected = FindNodeByPath(_selectedPath);
            _disableSelectionChangingEvent = true;
            RowsList.SelectedItem = selected;
            _disableSelectionChangingEvent = false;

            Dispatcher.UIThread.Post(() =>
            {
                if (RowsList?.FindDescendantOfType<ScrollViewer>() is { } scroller)
                    scroller.Offset = _scrollOffset;
            });

            if (vm != null)
                await vm.ViewRevisionFileAsync(selected?.Backend);
        }

        private ViewModels.RevisionFileTreeNode FindNodeByPath(string path)
        {
            ...
            {
                if (row.Backend?.Path.Equals(path, StringComparison.Ordinal) == true)
                    return row;
            }
        ...
        private async Task<List<ViewModels.RevisionFileTreeNode>> CreateNodesAsync(
            ViewModels.CommitDetail vm,
            string treeSHA,
            string parentPath,
            int depth,
            HashSet<string> availableExpandedPaths,
            string revision,
            long requestId)
        {
            var nodes = new List<ViewModels.RevisionFileTreeNode>();
            var objects = await vm.GetRevisionTreeChildrenAsync(treeSHA, parentPath);
            if (!IsCurrentReloadRequest(revision, requestId))
                return nodes;

            foreach (var obj in objects)
            {
                var node = new ViewModels.RevisionFileTreeNode
                {
                    Backend = obj,
                    Depth = depth,
                };

                if (obj.Type == Models.ObjectType.Tree && _expandedPaths.Contains(obj.Path))
                {
                    availableExpandedPaths.Add(obj.Path);
                    node.IsExpanded = true;
                    node.Children.AddRange(await CreateNodesAsync(vm, obj.SHA, obj.Path, depth + 1, availableExpandedPaths, revision, requestId));
                    if (!IsCurrentReloadRequest(revision, requestId))
                        return nodes;
                }
        ...
        private void BuildSearchResultTree(List<ViewModels.RevisionFileTreeNode> searchResult, Models.Object file)
        {
            var routes = file.Path.Split('/');
            if (routes.Length == 1)
            {
                searchResult.Add(new ViewModels.RevisionFileTreeNode
                {
                    Backend = file,
                });
                return;
            ...
            {
                prefix = string.IsNullOrEmpty(prefix) ? routes[i] : $"{prefix}/{routes[i]}";

                var folderNode = new ViewModels.RevisionFileTreeNode
                {
        ...
        private ContextMenu CreateRevisionFileContextMenu(ViewModels.Repository repo, ViewModels.CommitDetail vm, Models.Commit commit, Models.Object file)
        {
            ...
            if (openWith.IsEnabled)
            {
                ...
                {
                    await vm.OpenRevisionFileAsync(file.Path, null, file.SHA);
                    ev.Handled = true;
                };
                ...
                if (tools.Count > 0)
                {
                    ...
                    for (var i = 0; i < tools.Count; i++)
                    {
                        ...
                        {
                            await vm.OpenRevisionFileAsync(file.Path, tool, file.SHA);
                            ev.Handled = true;
                        };
            ...
            history.Click += (_, ev) =>
            {
                this.ShowWindow(new ViewModels.FileHistories(repo, file.Path, commit.SHA));
                ev.Handled = true;
            };
        ...
        private bool _isSearchResultMode = false;
        private string _searchResultPath = string.Empty;
        private long _reloadRequestId = 0;
        private long _searchRequestId = 0;
