...
namespace SourceGit.Views
{
    ...
    public partial class Histories : UserControl
    {
        ...
        protected override void OnAttachedToVisualTree(VisualTreeAttachmentEventArgs e)
        {
            ...
            SetObservedHistoriesViewModel(DataContext as ViewModels.Histories);
            UpdateDetailsPanelExpanded();
        }

        protected override void OnDetachedFromVisualTree(VisualTreeAttachmentEventArgs e)
        {
            ViewModels.Preferences.Instance.PropertyChanged -= OnPreferencesPropertyChanged;
            SetObservedHistoriesViewModel(null);
        ...
        private void SetObservedHistoriesViewModel(ViewModels.Histories vm)
        {
            if (_observedHistoriesViewModel == vm)
                return;

            if (_observedHistoriesViewModel != null)
                _observedHistoriesViewModel.PropertyChanged -= OnHistoriesViewModelPropertyChanged;

            _observedHistoriesViewModel = vm;
        ...
        {
            if (e.PropertyName == nameof(ViewModels.Histories.IsCollapseDetails))
                UpdateDetailsPanelExpanded();
        }

        private void OnPreferencesPropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            if (e.PropertyName == nameof(ViewModels.Preferences.UseTwoColumnsLayoutInHistories))
                UpdateDetailsPanelExpanded();
        }

        private void UpdateDetailsPanelExpanded()
        {
            var useHorizontal = ViewModels.Preferences.Instance.UseTwoColumnsLayoutInHistories;
            var isCollapsed = DataContext is ViewModels.Histories { IsCollapseDetails: true };
            SetCurrentValue(IsDetailsPanelExpandedProperty, useHorizontal || !isCollapsed);
        }
        ...
        private void OnTabHeaderPointerPressed(object sender, PointerPressedEventArgs e)
        {
            ...
            if (DataContext is ViewModels.Histories { IsCollapseDetails: true } vm)
                vm.IsCollapseDetails = false;
        }

        public void OnDetailsTabStripPointerPressed(object sender, PointerPressedEventArgs e)
        {
            ...
            if (!IsOnDetailsTabStripBlankArea(tabControl, e.GetPosition(tabControl)))
                return;

            var useHorizontal = ViewModels.Preferences.Instance.UseTwoColumnsLayoutInHistories;
            if (!useHorizontal && vm.IsCollapseDetails)
                vm.IsCollapseDetails = false;

            _isDraggingDetailsHeader = true;
            _detailsHeaderWasDragged = false;
            _isDraggingDetailsHeaderHorizontally = useHorizontal;

            if (useHorizontal)
            {
                _detailsHeaderDragStart = e.GetPosition(MainLayout).X;
                _detailsHeaderPrimaryStart = MainLayout.ColumnDefinitions[0].ActualWidth;
                _detailsHeaderSecondaryStart = MainLayout.ColumnDefinitions[2].ActualWidth;
            }
            else
            {
                _detailsHeaderDragStart = e.GetPosition(MainLayout).Y;
                _detailsHeaderPrimaryStart = MainLayout.RowDefinitions[0].ActualHeight;
                _detailsHeaderSecondaryStart = MainLayout.RowDefinitions[2].ActualHeight;
            }
        ...
        public void OnDetailsTabStripPointerMoved(object sender, PointerEventArgs e)
        {
            if (sender is not TabControl tabControl)
                return;

            if (!_isDraggingDetailsHeader)
            {
                var resizeCursor = GetDetailsResizeCursor();
                var targetCursor = IsOnDetailsTabStripBlankArea(tabControl, e.GetPosition(tabControl)) ? resizeCursor : Cursor.Default;
                if (tabControl.Cursor != targetCursor)
                    tabControl.Cursor = targetCursor;
            ...
            if (!e.GetCurrentPoint(tabControl).Properties.IsLeftButtonPressed)
            {
                StopDetailsHeaderDrag(e.Pointer);
                return;
            }

            if (_isDraggingDetailsHeaderHorizontally)
            {
                var columns = MainLayout.ColumnDefinitions;
                var delta = e.GetPosition(MainLayout).X - _detailsHeaderDragStart;
                var minDelta = columns[0].MinWidth - _detailsHeaderPrimaryStart;
                var maxDelta = _detailsHeaderSecondaryStart - columns[2].MinWidth;

                delta = Math.Clamp(delta, minDelta, maxDelta);
                columns[0].SetCurrentValue(
                    ColumnDefinition.WidthProperty,
                    new GridLength(_detailsHeaderPrimaryStart + delta, GridUnitType.Pixel));
                columns[2].SetCurrentValue(
                    ColumnDefinition.WidthProperty,
                    new GridLength(_detailsHeaderSecondaryStart - delta, GridUnitType.Pixel));
            }
            else
            {
                var rows = MainLayout.RowDefinitions;
                var delta = e.GetPosition(MainLayout).Y - _detailsHeaderDragStart;
                var minDelta = rows[0].MinHeight - _detailsHeaderPrimaryStart;
                var maxDelta = _detailsHeaderSecondaryStart - rows[2].MinHeight;

                delta = Math.Clamp(delta, minDelta, maxDelta);
                rows[0].SetCurrentValue(
                    RowDefinition.HeightProperty,
                    new GridLength(_detailsHeaderPrimaryStart + delta, GridUnitType.Pixel));
                rows[2].SetCurrentValue(
                    RowDefinition.HeightProperty,
                    new GridLength(_detailsHeaderSecondaryStart - delta, GridUnitType.Pixel));
            }

            _detailsHeaderWasDragged = true;
            e.Handled = true;
        }

        public void OnDetailsTabStripPointerReleased(object sender, PointerReleasedEventArgs e)
        {
            if (!_isDraggingDetailsHeader)
                return;
            ...
            if (sender is TabControl tabControl)
                tabControl.Cursor = Cursor.Default;

            e.Handled = _detailsHeaderWasDragged;
        }

        private void StopDetailsHeaderDrag(IPointer pointer)
        {
            _isDraggingDetailsHeader = false;
            pointer.Capture(null);
        }

        private Cursor GetDetailsResizeCursor()
        {
            return ViewModels.Preferences.Instance.UseTwoColumnsLayoutInHistories ? _resizingCursor : _detailsHeaderVerticalCursor;
        }

        private bool IsOnDetailsTabStripBlankArea(TabControl tabControl, Point point)
        {
            var hasHeader = false;
            var headerTop = double.MaxValue;
            var headerBottom = double.MinValue;

            foreach (var visual in tabControl.GetVisualDescendants())
            {
                if (visual is not TabItem tabItem || tabItem.FindAncestorOfType<TabControl>() != tabControl)
                    continue;

                var topLeft = tabItem.TranslatePoint(new Point(0, 0), tabControl);
                if (!topLeft.HasValue)
                    continue;

                var bounds = new Rect(topLeft.Value, tabItem.Bounds.Size);
                if (bounds.Contains(point))
                    return false;

                hasHeader = true;
                headerTop = Math.Min(headerTop, bounds.Top);
                headerBottom = Math.Max(headerBottom, bounds.Bottom);
            }

            return hasHeader && point.Y >= headerTop && point.Y <= headerBottom;
        }
        ...
        private Cursor _detailsHeaderVerticalCursor = new Cursor(StandardCursorType.TopSide);
        private ViewModels.Histories _observedHistoriesViewModel = null;
        private bool _isDraggingDetailsHeader = false;
        private bool _isDraggingDetailsHeaderHorizontally = false;
        private bool _detailsHeaderWasDragged = false;
        private double _detailsHeaderDragStart = 0;
        private double _detailsHeaderPrimaryStart = 0;
        private double _detailsHeaderSecondaryStart = 0;
    }
}
