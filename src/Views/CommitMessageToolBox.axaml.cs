...
namespace SourceGit.Views
{
        ...
        protected override void OnPropertyChanged(AvaloniaPropertyChangedEventArgs change)
        {
            ...
            if (change.Property == CommitMessageProperty)
            {
                ...
                for (var i = 0; i < lines.Length; i++)
                {
                    ...
                    subjectLen = line.TrimEnd().Length;
                    _subjectEndLine = i + 1;
                    break;
                }
    ...
    public partial class CommitMessageToolBox : UserControl
    {
        ...
        private async void OnOpenAIReview(object sender, RoutedEventArgs e)
        {
            if (DataContext is ViewModels.WorkingCopy vm && sender is Button button && ShowAdvancedOptions)
            {
                var repo = vm.Repository;
                ...
                if (vm.Staged == null || vm.Staged.Count == 0)
                {
                    repo.SendNotification(App.Text("AIReview.NoStagedChanges"), true);
                    e.Handled = true;
                    return;
                }
                ...
                if (services.Count == 1)
                {
                    DoOpenAIReview(repo, services[0], vm.Staged);
                    e.Handled = true;
                    return;
                }
                ...
                foreach (var service in services)
                {
                    ...
                    item.Click += (_, ev) =>
                    {
                        DoOpenAIReview(repo, dup, vm.Staged);
                        ev.Handled = true;
                    };
        ...
        private void DoOpenAIReview(ViewModels.Repository repo, AI.Service service, List<Models.Change> changes)
        {
            var owner = TopLevel.GetTopLevel(this) as Window;
            if (owner == null)
                return;

            var assistant = new ViewModels.AIAssistant(repo, service, changes, ViewModels.AIAssistant.Mode.Review);
            var view = new AIAssistant() { DataContext = assistant };
            view.Show(owner);
        }
