...
namespace SourceGit.Views
{
    public partial class Preferences : ChromelessWindow
    {
        ...
        public static readonly StyledProperty<Models.CustomDiffRenderer> SelectedCustomDiffRendererProperty =
            AvaloniaProperty.Register<Preferences, Models.CustomDiffRenderer>(nameof(SelectedCustomDiffRenderer));

        public Models.CustomDiffRenderer SelectedCustomDiffRenderer
        {
            get => GetValue(SelectedCustomDiffRendererProperty);
            set => SetValue(SelectedCustomDiffRendererProperty, value);
        }
        ...
        public Preferences(int selectedTabIndex)
        {
        ...
        private void OnAddCustomDiffRenderer(object sender, RoutedEventArgs e)
        {
            var renderer = new Models.CustomDiffRenderer()
            {
                Name = "Unity Prefab",
            ...
            ViewModels.Preferences.Instance.CustomDiffRenderers.Add(renderer);
            SelectedCustomDiffRenderer = renderer;

            e.Handled = true;
        }

        private async void SelectExecutableForCustomDiffRenderer(object sender, RoutedEventArgs e)
        {
            var options = new FilePickerOpenOptions()
            {
                FileTypeFilter = [new FilePickerFileType("Executable file(script)") { Patterns = ["*.*"] }],
            ...
            {
                var selected = await StorageProvider.OpenFilePickerAsync(options);
                if (selected is { Count: 1 } && sender is Button { DataContext: Models.CustomDiffRenderer renderer })
                    renderer.Executable = selected[0].Path.LocalPath;
            }
            catch (Exception ex)
            {
                await new Alert().ShowAsync(this, $"Failed to select executable for custom diff renderer: {ex.Message}", true);
            }

            e.Handled = true;
        }

        private void OnRemoveSelectedCustomDiffRenderer(object sender, RoutedEventArgs e)
        {
            if (SelectedCustomDiffRenderer == null)
                return;

            ViewModels.Preferences.Instance.CustomDiffRenderers.Remove(SelectedCustomDiffRenderer);
            SelectedCustomDiffRenderer = null;
            e.Handled = true;
        }

        private void OnMoveSelectedCustomDiffRendererUp(object sender, RoutedEventArgs e)
        {
            if (SelectedCustomDiffRenderer == null)
                return;
        ...
        private void OnMoveSelectedCustomDiffRendererDown(object sender, RoutedEventArgs e)
        {
            if (SelectedCustomDiffRenderer == null)
                return;
