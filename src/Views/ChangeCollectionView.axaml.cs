...
namespace SourceGit.Views
{
    ...
    public class ChangeCollectionContainer : ListBoxEx
    {
        ...
        protected override void OnKeyDown(KeyEventArgs e)
        {
            if (SelectedItems is [ViewModels.ChangeTreeNode node] && e.KeyModifiers == KeyModifiers.None)
            {
                ...
                else if (e.Key is Key.W or Key.S)
                {
        ...
        private ViewModels.ChangeTreeNode FindVisibleFile(ViewModels.ChangeTreeNode item, int step)
        {
            var idx = Items.IndexOf(item);
            if (idx == -1)
                return null;
            ...
            {
                if (Items[i] is ViewModels.ChangeTreeNode { IsFolder: false } node)
                    return node;
            }
    ...
    public partial class ChangeCollectionView : UserControl
    {
        ...
        public ViewModels.ChangeTreeNode GetSelectedSingleFolder()
        {
        ...
        public List<Models.Change> GetSelectedChangesInSingleFolder()
        {
            var selected = new List<Models.Change>();
            if (GetSelectedSingleFolder() is { } node)
                CollectChangesInNode(selected, node);

            return selected;
        }

        public List<Models.Change> GetSelectedChangesIncludingFolders()
        {
            var selected = new List<Models.Change>();
            var container = this.FindDescendantOfType<ChangeCollectionContainer>();
            if (container is not { SelectedItems: { } selectedItems })
                return selected;

            foreach (var item in selectedItems)
            {
                if (item is Models.Change c)
                    AddChange(selected, c);
        ...
        public List<string> GetSelectedPaths()
        {
            ...
            {
                if (item is Models.Change c)
                    AddPath(paths, c.Path);
                else if (item is ViewModels.ChangeTreeNode node)
                    AddPath(paths, node.FullPath);
            }
        ...
        public Models.Change GetNextChangeWithoutSelection(List<Models.Change> selectedChanges = null)
        {
            var selected = selectedChanges ?? SelectedChanges;
            var changes = Changes;
        ...
        private void OnRowSelectionChanged(object sender, SelectionChangedEventArgs _)
        {
            ...
            if (sender is ListBox { SelectedItems: { } selectedItems })
            {
                foreach (var item in selectedItems)
                {
                    if (item is Models.Change c)
                        AddChange(selected, c);
                    else if (item is ViewModels.ChangeTreeNode { IsFolder: false } node && !selected.Contains(node.Change))
                        AddChange(selected, node.Change);
                }
            }
        ...
        private void AddChange(List<Models.Change> outs, Models.Change change)
        {
            if (change != null && !outs.Contains(change))
                outs.Add(change);
        }

        private void AddPath(List<string> outs, string path)
        {
            if (!string.IsNullOrEmpty(path) && !outs.Contains(path))
                outs.Add(path);
        }
