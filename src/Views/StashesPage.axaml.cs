...
namespace SourceGit.Views
{
    public partial class StashesPage : UserControl
    {
        ...
        private void OnChangesHeaderDragAreaPointerPressed(object sender, PointerPressedEventArgs e)
        {
            ...
            _isDraggingChangesHeader = true;
            _changesHeaderDragStartY = e.GetPosition(LeftPane).Y;
            _stashListStartHeight = LeftPane.RowDefinitions[2].ActualHeight;
            _changesStartHeight = LeftPane.RowDefinitions[4].ActualHeight;
            e.Pointer.Capture(control);
            e.Handled = true;
        }

        private void OnChangesHeaderDragAreaPointerMoved(object sender, PointerEventArgs e)
        {
            if (!_isDraggingChangesHeader)
                return;

            var rows = LeftPane.RowDefinitions;
            var delta = e.GetPosition(LeftPane).Y - _changesHeaderDragStartY;
            var minDelta = rows[2].MinHeight - _stashListStartHeight;
            var maxDelta = _changesStartHeight - rows[4].MinHeight;

            delta = Math.Clamp(delta, minDelta, maxDelta);
            rows[2].Height = new GridLength(_stashListStartHeight + delta, GridUnitType.Pixel);
            rows[4].Height = new GridLength(_changesStartHeight - delta, GridUnitType.Pixel);
            e.Handled = true;
        }

        private void OnChangesHeaderDragAreaPointerReleased(object sender, PointerReleasedEventArgs e)
        {
            if (!_isDraggingChangesHeader)
                return;

            _isDraggingChangesHeader = false;
        ...
        private bool _isDraggingChangesHeader = false;
        private double _changesHeaderDragStartY = 0;
        private double _stashListStartHeight = 0;
        private double _changesStartHeight = 0;
    }
}
