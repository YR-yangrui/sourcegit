...
namespace SourceGit.Commands
{
    public partial class QuerySubmodules : Command
    {
        ...
        public QuerySubmodules(string repo, bool includeUntracked)
        {
            WorkingDirectory = repo;
            Context = repo;
            Args = OperatingSystem.IsWindows() ? "submodule--helper status --" : "submodule status";
        ...
        public async Task<List<Models.Submodule>> GetResultAsync()
        {
            ...
            var rs = await ReadStatusAsync().ConfigureAwait(false);
            ...
            if (needCheckLocalChanges)
            {
                ...
                Args = $"--no-optional-locks {GetStatusUntrackedArg(_includeUntracked)} --porcelain -- {builder}";
                rs = await ReadToEndAsync().ConfigureAwait(false);
                if (!rs.IsSuccess)
                    return submodules;
        ...
        private async Task<Result> ReadStatusAsync()
        {
            var rs = await ReadToEndAsync().ConfigureAwait(false);
            if (rs.IsSuccess || !OperatingSystem.IsWindows())
                return rs;
