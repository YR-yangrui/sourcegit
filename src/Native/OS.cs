...
namespace SourceGit.Native
{
    public static partial class OS
    {
        ...
        public static void LogException(Exception ex)
        {
            ...
            SourceGit.Diagnostics.DiagnosticManager.Error("Crash", "crash.captured", "Crash captured", ex);
            ...
            var traceFile = SourceGit.Diagnostics.DiagnosticManager.ExportRecentPerfettoTrace(Path.Combine(crashDir, $"{time}.perfetto.json"));
            ...
            if (!string.IsNullOrEmpty(traceFile))
                writer.WriteLine($"Perfetto Trace: {traceFile}");
            writer.WriteLine("----------------------------");
            writer.WriteLine();
            writer.WriteLine(ex);
            writer.WriteLine();
            writer.WriteLine("----------------------------");
            writer.WriteLine("Recent Diagnostics");
            writer.WriteLine("----------------------------");
            SourceGit.Diagnostics.DiagnosticManager.WriteRecentEvents(writer, 80);
            writer.Flush();
        }
