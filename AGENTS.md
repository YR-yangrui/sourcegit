...
- 发布示例：`dotnet publish src/SourceGit.csproj -c Release -r win-x64 -o publish`。
- 完成代码修改后默认打 Windows exe：`dotnet publish src/SourceGit.csproj -c Release -r win-x64 -o publish`，产物为 `publish/SourceGit.exe`。
